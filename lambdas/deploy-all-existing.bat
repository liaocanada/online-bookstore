...
for %%f in (*.js) do if not "%%~f"=="runner.js" (
    ...
    aws lambda update-function-configuration --function-name "!toDeploy:~0,-3!" --environment "Variables={NODE_PATH=/opt/lambdas/node_modules}" --timeout 90 --layers "arn:aws:lambda:us-east-1:095371326078:layer:bookstore-layer-aurora:%layerVersion%"
)
